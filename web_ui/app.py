...
class StreamingLogHandler:
    ...
    def emit(self, message):
        ...
        log_entry = {"timestamp": timestamp, "message": message, "type": "info"}
        self.logs.put(log_entry)

    def emit_error(self, message):
        """Add an error message to the queue"""
        timestamp = datetime.now().strftime("%H:%M:%S")
        log_entry = {"timestamp": timestamp, "message": message, "type": "error"}
        self.logs.put(log_entry)
...
def index():
    ...
    if not db:
        return render_template(
            "error.html",
            error="Database connection failed. Please check if the database exists.",
        )
    ...
    try:
        ...
        return render_template(
            "index.html", stats=stats, language_stats=language_stats[:10]
        )
    except Exception as e:
    ...
    try:
        ...
        for rec in recommendations[:5]:  # Top 5 recommendations
            ...
            if total_required > 0:
                ...
                if image_name:
                    installed_packages_and_managers = (
                        recommendation_engine.get_system_packages_and_package_managers(
                            image_name
                        )
                    )
...
def api_scan():
    ...
    try:
        ...
        if update_existing:
            ...
            print(
                "⏭️  Skip mode: Will skip images already in database (use update_existing to rescan)"
            )
            print(
...
def api_stats():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "stats": stats, "language_stats": language_stats[:15]}
        )
        ...
        return jsonify(
            {
                ...
                "images": results[
                    :20
                ],  # Limit to 20 results, use 'images' key to match frontend
            }
        )
...
def api_get_all_images():
    ...
    try:
        ...
        if search_query:
            ...
            images = db.get_all_images_with_details(
                page=page, per_page=per_page, language_filter="", security_filter="all"
            )
...
def api_get_image_by_name(image_name):
    ...
    try:
        ...
        cursor = db.conn.execute(
            "SELECT name FROM images WHERE name LIKE ? LIMIT 3",
            (f"%{image_name.split('/')[-1]}%",),
        )
        ...
        if image:
            ...
            return jsonify({"success": True, "image": image, "found": True})
        else:
...
def api_compare_images():
    ...
    try:
        ...
        if not image1_id or not image2_id:
            return jsonify(
                {"success": False, "error": "Two image IDs are required for comparison"}
            )

        if image1_id == image2_id:
            return jsonify(
                {"success": False, "error": "Cannot compare an image with itself"}
            )
        ...
        if not image1:
            return jsonify(
                {"success": False, "error": f"Image with ID {image1_id} not found"}
            )
        if not image2:
            return jsonify(
                {"success": False, "error": f"Image with ID {image2_id} not found"}
            )
        ...
        for pkg_name in all_package_names:
            if pkg_name in packages1_set and pkg_name in packages2_set:
                ...
                if version1 == version2:
                    common_packages.append({"name": pkg_name, "version": version1})
                else:
                    different_versions.append(
                        {"name": pkg_name, "version1": version1, "version2": version2}
                    )
            elif pkg_name in packages1_set:
                unique_to_image1.append(
                    {"name": pkg_name, "version": packages1_set[pkg_name]}
                )
            else:
                unique_to_image2.append(
                    {"name": pkg_name, "version": packages2_set[pkg_name]}
                )
        ...
        if image1.get("size_bytes") and image2.get("size_bytes"):
            ...
            size_diff_percent = (
                (size_diff / image1["size_bytes"]) * 100
                if image1["size_bytes"] > 0
                else 0
            )

        vuln_diff = {
            "total": (
                image2.get("total_vulnerabilities", 0)
                - image1.get("total_vulnerabilities", 0)
            ),
            "critical": (
                image2.get("critical_vulnerabilities", 0)
                - image1.get("critical_vulnerabilities", 0)
            ),
            "high": (
                image2.get("high_vulnerabilities", 0)
                - image1.get("high_vulnerabilities", 0)
            ),
            "medium": (
                image2.get("medium_vulnerabilities", 0)
                - image1.get("medium_vulnerabilities", 0)
            ),
            "low": (
                image2.get("low_vulnerabilities", 0)
                - image1.get("low_vulnerabilities", 0)
            ),
        }

        # Determine which image is "better" based on security and size
        security_score1 = (
            image1.get("critical_vulnerabilities", 0) * 4
        ...
        security_score2 = (
            image2.get("critical_vulnerabilities", 0) * 4
        ...
        recommendation = {
            ...
            "size_winner": (
                "image1"
                if (image1.get("size_bytes", 0) < image2.get("size_bytes", 0))
                else (
                    "image2"
                    if (image2.get("size_bytes", 0) < image1.get("size_bytes", 0))
                    else "tie"
                )
            ),
    ...
    try:
        ...
        def run_scan():
            ...
            try:
                ...
                log_handler.emit(
                    f"📋 Configuration: comprehensive={comprehensive}, update_existing={update_existing}, max_tags={max_tags}"
                )
                ...
                if update_existing:
                    log_handler.emit(
                        "🔄 Update mode: Will rescan and update existing images in database"
                    )
                else:
                    log_handler.emit(
                        "⏭️  Skip mode: Will skip images already in database"
                    )
                ...
                try:
                    ...
                    try:
                        ...
                        log_handler.emit(
                            f"   • Total images: {db_stats.get('total_images', 0)}"
                        )
                        log_handler.emit(
                            f"   • Zero vulnerability images: {db_stats.get('zero_vuln_images', 0)}"
                        )
                        log_handler.emit(
                            f"   • Safe images (no critical/high): {db_stats.get('safe_images', 0)}"
                        )
                        log_handler.emit(
                            f"   • Average vulnerabilities: {db_stats.get('avg_vulnerabilities', 0):.1f}"
                        )
                        ...
                        if language_stats:
                            ...
                            for lang_stat in language_stats[:5]:
                                log_handler.emit(
                                    f"   • {lang_stat['language']}: {lang_stat['image_count']} images (avg {lang_stat['avg_vulnerabilities']:.1f} vulns)"
                                )

                    except Exception as e:
                        log_handler.emit(
                            f"⚠️  Could not retrieve database statistics: {e}"
                        )
        ...
        def run_repo_scan():
            ...
            try:
                ...
                try:
                    ...
                    log_handler.emit(
                        f"✅ Repository scan completed! Analyzed {len(results)} images"
                    )
                    ...
                    if len(results) == 0:
                        ...
                        log_handler.emit(
                            "   • All images were skipped (already in database)"
                        )
                    ...
                    try:
                        if len(results) > 0:
                            ...
                            total_vulns = sum(
                                r.get("vulnerabilities", {}).get("total", 0)
                                for r in results
                            )
                            avg_vulns = (
                                total_vulns / len(results) if len(results) > 0 else 0
                            )
                            log_handler.emit(f"   • Images analyzed: {len(results)}")
                            log_handler.emit(
                                f"   • Average vulnerabilities: {avg_vulns:.1f}"
                            )
                            ...
                            critical_count = sum(
                                1
                                for r in results
                                if r.get("vulnerabilities", {}).get("critical", 0) > 0
                            )
                            high_count = sum(
                                1
                                for r in results
                                if r.get("vulnerabilities", {}).get("high", 0) > 0
                            )
                            safe_count = len(results) - critical_count - high_count

                            log_handler.emit(
                                f"   • Images with critical vulnerabilities: {critical_count}"
                            )
                            log_handler.emit(
                                f"   • Images with high vulnerabilities: {high_count}"
                            )
                            log_handler.emit(
                                f"   • Safe images (no critical/high): {safe_count}"
                            )
    ...
    try:
        ...
        def run_image_scan():
            ...
            try:
                ...
                log_handler.emit(
                    f"📋 Configuration: comprehensive={comprehensive}, update_existing={update_existing}"
                )
                ...
                # Try both the original name and normalized Docker names
                def check_image_exists_normalized(image_name):
                    """Check if image exists using various name forms"""
                    if scanner.db.image_exists(image_name):
                        return image_name

                    # Try with docker.io/library prefix for standard images
                    if not image_name.startswith(
                        ("docker.io/", "mcr.microsoft.com/", "registry", "localhost")
                    ):
                        if (
                            "/" not in image_name
                        ):  # Standard library image like python:3.12
                            normalized_name = f"docker.io/library/{image_name}"
                            if scanner.db.image_exists(normalized_name):
                                return normalized_name
                        else:  # User/org image like user/image:tag
                            normalized_name = f"docker.io/{image_name}"
                            if scanner.db.image_exists(normalized_name):
                                return normalized_name

                    return None

                # Store the original image name to ensure it's always available
                original_image_name = image_name
                scan_image_name = image_name  # This will be the name used for scanning

                existing_image_name = check_image_exists_normalized(image_name)
                if existing_image_name:
                    if not update_existing:
                        log_handler.emit(
                            f"📋 Image {original_image_name} already exists in database (stored as: {existing_image_name})"
                        )
                        log_handler.emit(
                            "✅ Returning existing analysis data (use 'Update Existing' to rescan)"
                        )

                        # Return the existing image data instead of failing
                        existing_image = scanner.db.get_image_by_exact_name(
                            existing_image_name
                        )
                        if existing_image:
                            log_handler.emit(
                                f"📊 Found existing analysis with {existing_image.get('total_vulnerabilities', 0)} vulnerabilities"
                            )
                            # Handle case where languages might be None
                            languages = existing_image.get("languages", []) or []
                            language_names = [
                                lang.get("language", "unknown")
                                for lang in languages
                                if lang and isinstance(lang, dict)
                            ]
                            if language_names:
                                log_handler.emit(
                                    f"🏷️  Languages: {', '.join(language_names)}"
                                )
                            else:
                                log_handler.emit("🏷️  Languages: None detected")
                            log_handler.complete(success=True)
                            scanner.db.close()
                            return
                        else:
                            log_handler.emit(
                                "⚠️  Image exists but could not retrieve details, proceeding with fresh scan..."
                            )
                            # Keep using the existing name for consistency
                            scan_image_name = existing_image_name
                    else:
                        log_handler.emit(
                            f"🔄 Image exists (stored as: {existing_image_name}), updating with update_existing flag..."
                        )
                        # Use the existing name for consistency
                        scan_image_name = existing_image_name

                # Analyze the image
                log_handler.emit("🔍 Analyzing image structure...")
                analyzer = ImageAnalyzer(scan_image_name)
                analysis = analyzer.analyze()

                # Add Docker manifest/inspect data for size and layers
                log_handler.emit("📊 Getting image manifest data...")
                manifest_data = get_docker_manifest_data(scan_image_name, log_handler)
                if manifest_data:
                ...
                if comprehensive:
                    ...
                    log_handler.emit(
                        "⚡ Running fast vulnerability scan (Trivy vulnerabilities only)..."
                    )

                vulnerability_data = scanner.scan_vulnerabilities(
                    scan_image_name, comprehensive
                )
                ...
                image_id = scanner.db.insert_image_analysis(
                    analysis, force_update=force_update
                )
                analysis["database_id"] = image_id

                log_handler.emit(
                    f"✅ Image successfully added to database with ID: {image_id}"
                )
                ...
                if "comprehensive_security" in analysis:
                    ...
                    log_handler.emit(
                        f"   � Secrets found: {comp_data.get('secrets_found', 0)}"
                    )
                    log_handler.emit(
                        f"   ⚙️  Configuration issues: {comp_data.get('config_issues', 0)}"
                    )
                    log_handler.emit(
                        f"   📋 License issues: {comp_data.get('license_issues', 0)}"
                    )

                # Clean up the Docker image if cleanup is enabled
                if scanner.cleanup_images:
                    log_handler.emit("🧹 Cleaning up Docker images...")
                    scanner.cleanup_docker_images([scan_image_name])
...
def api_analyze_and_recommend():
    ...
    try:
        ...
        if not re.match(
            r"^[a-zA-Z0-9._/-]+:[a-zA-Z0-9._-]+$|^[a-zA-Z0-9._/-]+$", image_name
        ):
        ...
        (
            ...
            recommendations,
            final_requirement,
        ) = recommendation_engine.recommend_from_existing_image(image_name, requirement)

        print(f"Found {len(recommendations)} recommendations")
        if final_requirement:
            print(
                f"Final search criteria - Language: {final_requirement.language}, Version: {final_requirement.version}"
            )
...
def get_docker_manifest_data(image_name: str, log_handler) -> Optional[Dict]:
    ...
    try:
        ...
        manifest_data = {
            ...
            "digest": (
                inspect_data.get("RepoDigests", [None])[0]
                if inspect_data.get("RepoDigests")
                else None
            ),
        }
        ...
        if actual_size > 0:
            ...
            log_handler.emit(
                f"   📊 Size: Unknown (0 bytes), Layers: {manifest_data['layers']}"
            )
