...
class ImageAnalyzer:
    ...
    def __init__(self, image_name: str):
        ...
        self.excluded_packages = {
            'python': [
                'python-wheel', 'python-pip', 'python-setuptools', 'python-dev',
                'python-distutils', 'python-pkg-resources', 'python-six',
                'python-urllib3', 'python-requests', 'python-chardet',
                'python-certifi', 'python-idna', 'python-pysocks'
            ],
            'node': [
                'nodejs-dev', 'nodejs-npm', 'node-gyp'
            ],
            'java': [
                'java-common'
            ]
    ...
    def analyze(self) -> Dict:
        ...
        if not self.syft_data:
            print("Warning: Syft analysis failed, proceeding with runtime verification only")
    ...
    def extract_languages_from_syft(self) -> List[Dict]:
        ...
        print(f"  📦 Analyzing {len(self.syft_data['artifacts'])} packages for language runtimes...")
            ...
            for lang, patterns in self.language_patterns.items():
                ...
                for pattern in patterns:
                    if re.match(pattern, package_name):
                        ...
                        priority = self.calculate_package_priority(package_name, lang, package_type)
                        
                        # Only keep if this is a better match than what we already have
                        if lang not in found_languages or priority > found_languages[lang]['priority']:
                            
                            print(f"    🔍 Found {lang} runtime: {original_name} v{version} (priority: {priority})")
                            ...
                            if 'metadata' in artifact:
                                ...
                                if 'architecture' in metadata:
                                    runtime_info['architecture'] = metadata['architecture']
                                if 'vendor' in metadata:
                                    runtime_info['vendor'] = metadata['vendor']
                            
                            # Special handling for Python versions
                            if lang == 'python':
                                runtime_info['major_minor'] = self.extract_python_version(package_name, version)
                                print(f"      🐍 Extracted Python major.minor: {runtime_info['major_minor']}")
                            
                            # Special handling for Node.js versions
                            elif lang == 'node':
                                runtime_info['major_minor'] = self.extract_nodejs_version(package_name, version)
                                print(f"      🟢 Extracted Node.js major.minor: {runtime_info['major_minor']}")
    ...
    def calculate_package_priority(self, package_name: str, language: str, package_type: str) -> int:
        ...
        if any(suffix in package_name for suffix in ['-dev', '-devel', '-lib', '-common']):
            priority -= 20
    ...
    def verify_runtime_versions(self) -> List[Dict]:
        ...
        for runtime, cmd in self.runtime_commands.items():
            try:
                ...
                if result.returncode == 0:
                    version = self.parse_version_output(runtime, result.stdout, result.stderr)
                    if version:
    ...
    def parse_version_output(self, runtime: str, stdout: str, stderr: str) -> Optional[str]:
        ...
        for runtime in self.verified_runtimes:
            ...
            if key in language_map:
                ...
                syft_info.update({
                    ...
                    'verified': True,
                    'version': runtime['version']  # Use verified version as primary
                })
...
def main():
    ...
    if len(sys.argv) != 2:
        ...
        print("Example: python image_analyzer.py mcr.microsoft.com/azurelinux/base/python:3.12")
        sys.exit(1)
