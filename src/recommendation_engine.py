...
class RecommendationEngine:
    ...
    def __init__(self, database_path: str = "azure_linux_images.db"):
        ...
        if not self.logger.handlers:
            ...
            self.logger.setLevel(
                logging.INFO
            )  # Set to INFO to see package ecosystem logs
    ...
    def _filter_platform_specific_images(
        self, candidate_images: List[Dict]
    ) -> List[Dict]:
        ...
        for image in candidate_images:
            ...
            for keyword in platform_keywords:
                ...
                if re.search(pattern, image_name):
                    ...
                    self.logger.debug(
                        f"Found platform keyword '{keyword}' in image: {image.get('name', '')}"
                    )
                    break
            ...
            if not has_platform_keyword:
                ...
                self.logger.debug(
                    f"Filtering out platform-specific image: {image.get('name', '')}"
                )

        self.logger.info(
            f"Filtered {len(candidate_images) - len(filtered_images)} platform-specific images, {len(filtered_images)} remaining"
        )
    ...
    def recommend(self, requirements: UserRequirement) -> List[ImageRecommendation]:
        ...
        if requirements.security_level == "maximum":
            candidate_images = [
                ...
                if img["critical_vulnerabilities"] == 0
                and img["high_vulnerabilities"] == 0
            ]
        elif requirements.security_level == "high":
            candidate_images = [
                img
                for img in candidate_images
                if img["critical_vulnerabilities"]
                <= requirements.max_critical_vulnerabilities
        ...
        if not candidate_images:
            print(
                "No images found matching security, language, and platform requirements"
            )
            return []
        ...
        for image_data in candidate_images:
            ...
            if score > 0:  # Only include viable options
                recommendation = ImageRecommendation(
                    ...
                    package_compatibility=self.calculate_package_compatibility(
                        analysis_data, requirements
                    ),
    ...
    def _convert_db_row_to_analysis(self, image_row: Dict) -> Dict:
        ...
        analysis_data = {
            ...
            "languages": (
                ...
                if image_row.get("language")
                else []
            ),
            "capabilities": capabilities,
            "manifest": {"size": image_row.get("size_bytes", 0)},
            "vulnerabilities": {
            ...
            "base_os": {"name": image_row.get("base_os_name", "azurelinux")},
        }

        return analysis_data

    def score_image(
        self, image_data: Dict, requirements: UserRequirement
    ...
    def score_language_match(
        ...
        base_os = (
            image_data.get("base_os", {}).get("name", "") or ""
    ...
    def score_version_compatibility(
        self, image_data: Dict, requirements: UserRequirement
    ) -> float:
        ...
        for lang_info in languages:
            if lang_info["language"].lower() == target_lang:
                ...
                return self.compare_versions(
                    requirements.version, image_version, target_lang
                )
    ...
    def score_package_ecosystem(
        ...
        logger.info(
            f"Scoring package ecosystem for image: {image_data.get('image', 'unknown')}"
        )
        ...
        installed_packages_and_managers = self.get_system_packages_and_package_managers(
            image_name
        )

        logger.info(f"Available package managers in image: {list(available_managers)}")
        logger.info(
            f"Available system packages count: {len(installed_system_packages)}"
        )
        logger.info(
            f"Total installed packages/managers: {len(installed_packages_and_managers)}"
        )
        ...
        for package in requirements.packages:
            ...
            if package_lower in installed_packages_and_managers:
                ...
                logger.debug(
                    f"Package '{package}' not found in installed packages/managers"
                )
    ...
    def get_installed_package_managers(self, image_name: str) -> set:
        ...
        logger.debug(
            f"Found {len(package_managers)} package managers: {list(package_managers)}"
        )
    ...
    def check_installed_packages(
        self, image_name: str, required_packages: List[str]
    ) -> float:
    ...
    def score_security(self, image_data: Dict, requirements: UserRequirement) -> float:
        ...
        is_azure_linux = (
            "azurelinux" in base_os.lower()
            or "azurelinux" in image_name.lower()
            or "mcr.microsoft.com/azurelinux" in image_name.lower()
        )
    ...
    def calculate_package_compatibility(
        self, image_data: Dict, requirements: UserRequirement
    ) -> float:
    ...
    def format_recommendations(
        self, recommendations: List[ImageRecommendation], limit: int = 5
    ) -> str:
    ...
    def get_package_reasoning(
        ...
        if total_found == total_packages:
            ...
            can_install = any(
                expected in available_managers for expected in expected_managers
            )
            if can_install:
    ...
    def get_image_from_database(self, image_name: str) -> Optional[Dict]:
        ...
        cursor = self.db.conn.execute(
            "SELECT language, version, major_minor, verified FROM languages WHERE image_id = ?",
            (image_id,),
        )
        languages = []
        ...
        self.logger.debug(
            f"Found {len(languages)} languages in languages table: {[l['language'] for l in languages]}"
        )
        ...
        if not languages and image_dict.get("language"):
            self.logger.debug(
                f"No languages in languages table, using main table fallback: {image_dict.get('language')}"
            )
            languages = [
                {
        ...
        analysis_data = {
            ...
            "manifest": {"size": image_dict.get("size_bytes", 0)},
            "vulnerabilities": {
            ...
            "base_os": {"name": image_dict.get("base_os_name", "azurelinux")},
        }

        return analysis_data

    def recommend_from_existing_image(
        self, image_name: str, requirements: UserRequirement
    ) -> Tuple[Optional[Dict], List[ImageRecommendation], UserRequirement]:
        """Get recommendations based on an existing image in the database"""

        self.logger.info(
            f"=== Starting recommendation analysis for existing image: {image_name} ==="
        )
        self.logger.info(
            f"Input requirements - Size: {requirements.size_preference}, Security: {requirements.security_level}"
        )
        self.logger.info(
            f"Vulnerability limits - Max total: {requirements.max_vulnerabilities}, Critical: {requirements.max_critical_vulnerabilities}, High: {requirements.max_high_vulnerabilities}"
        )
        ...
        if not analysis_data:
            ...
            self.logger.info(
                "Recommendation: Use the scan feature to add this image to the database first"
            )
            return None, [], requirements

        self.logger.info(f"✅ Found image in database: {image_name}")
        self.logger.info(
            f"Image size: {analysis_data.get('manifest', {}).get('size', 0)} bytes"
        )
        ...
        language_descriptions = [
            f"{l.get('language')} {l.get('version', 'no-version')}"
            for l in detected_languages
        ]
        self.logger.info(
            f"Detected {len(detected_languages)} languages: {language_descriptions}"
        )
        self.logger.info(
            f"Found {len(system_packages)} system packages and {len(package_managers)} package managers"
        )
        self.logger.info(
            f"Vulnerability summary - Total: {vulnerabilities.get('total', 0)}, Critical: {vulnerabilities.get('critical', 0)}, High: {vulnerabilities.get('high', 0)}"
        )

        if not detected_languages:
            self.logger.error(
                "❌ No programming language runtimes detected in the stored image analysis"
            )
        ...
        self.logger.info(
            f"Default primary language: {primary_language.get('language')} {primary_language.get('version', 'no-version')}"
        )
        ...
        for lang in detected_languages:
            if lang.get("verified", False):
                ...
                self.logger.info(
                    f"Found verified language, switching to: {primary_language.get('language')} {primary_language.get('version', 'no-version')}"
                )
                break
        ...
        if not primary_language.get("verified", False):
            for lang in detected_languages:
                if lang.get("version"):
                    primary_language = lang
                    self.logger.info(
                        f"Found language with version, switching to: {primary_language.get('language')} {primary_language.get('version', 'no-version')}"
                    )
                    break
        ...
        self.logger.info(
            f"Language verification status: {'verified' if primary_language.get('verified', False) else 'unverified'}"
        )
        ...
        self.logger.info(
            f"Using first {len(limited_packages)} packages (limit: {package_limit}) for recommendations"
        )
        ...
        self.logger.info(
            f"Step 6: Running recommendation engine with primary criteria..."
        )
        recommendations = self.recommend(updated_requirements)
        self.logger.info(
            f"Primary search result: {len(recommendations)} recommendations found"
        )

        # If no recommendations found with specific version, try with major.minor only
        if not recommendations and version:
            self.logger.info(
                f"Step 7: No results with specific version, trying fallback strategies..."
            )
            import re
            ...
            if version_match:
                ...
                self.logger.info(
                    f"Fallback 1: Trying with major.minor version only: {version} -> {major_minor}"
                )
                ...
                self.logger.info(
                    f"Fallback 1 result: {len(recommendations)} recommendations found"
                )
                if recommendations:
                    updated_requirements = fallback_requirements  # Use the fallback requirements for response
                    self.logger.info(
                        f"✅ Using fallback requirements with major.minor version"
                    )

        # If still no recommendations, try without version constraint
        if not recommendations:
            self.logger.info(
                f"Fallback 2: No results with version constraint, trying without version"
            )
            ...
            self.logger.info(
                f"Fallback 2 result: {len(recommendations)} recommendations found"
            )
            if recommendations:
                updated_requirements = no_version_requirements  # Use the no-version requirements for response
                self.logger.info(
                    f"✅ Using fallback requirements without version constraint"
                )
        ...
        if recommendations:
            top_scores = [
                f"{rec.image_name} ({rec.score:.3f})" for rec in recommendations[:3]
            ]
            self.logger.info(f"Top 3 recommendations by score: {top_scores}")
        else:
            self.logger.warning(
                f"⚠️ No recommendations found even with fallback strategies"
            )
