...
Examples:
  ...
  %(prog)s --scan --update-db --max-tags 0  # Scan ALL tags (can be slow)
    ...
    operation.add_argument(
        '--analyze', '-a',
        metavar='IMAGE',
        help='Analyze a specific container image'
    )
    operation.add_argument(
        '--scan', '-s',
        action='store_true',
    ...
    rec_group.add_argument(
        '--version', '-v',
        help='Language version (e.g., 3.12, 18, 17)'
    )
    rec_group.add_argument(
        '--packages', '-p',
    ...
    db_group.add_argument(
        '--update-db',
        action='store_true',
        help='Update database during scan'
    )
    db_group.add_argument(
        '--comprehensive',
        action='store_true',
...
def handle_analyze(args) -> int:
    ...
    try:
        ...
        vulnerability_data = scanner.scan_vulnerabilities(args.analyze, args.comprehensive)
...
def handle_scan_image(args) -> int:
    ...
    try:
        ...
        logger.info(f"Scanner configuration: comprehensive={args.comprehensive}, cleanup={not args.no_cleanup}, update_existing={args.update_existing}")
        ...
        try:
            ...
            logger.info(f"Database statistics: {db_stats.get('total_images', 0)} total images")
            print(f"\n📊 Database Statistics:")
            ...
            print(f"  Average vulnerabilities per image: {db_stats.get('avg_vulnerabilities', 0):.1f}")
...
def handle_scan_repo(args) -> int:
    ...
    try:
        ...
        logger.info(f"Scanner configuration: comprehensive={args.comprehensive}, cleanup={not args.no_cleanup}, update_existing={args.update_existing}, max_tags={args.max_tags or 'unlimited'}")
...
def handle_scan_all_mcr(args) -> int:
    ...
    if args.update_existing:
        ...
        print("⏭️  Skip mode: Will skip images already in database (use --update-existing to rescan)")
    ...
    try:
        ...
        try:
            stats = scanner.get_database_stats()
            db_stats = stats['database_stats']
            logger.info(f"Final database statistics: {db_stats.get('total_images', 0)} total images")
            print(f"\n📊 Database Statistics:")
            print(f"  Total images in database: {db_stats.get('total_images', 0)}")
            print(f"  Zero vulnerability images: {db_stats.get('zero_vuln_images', 0)}")
            print(f"  Safe images (no critical/high): {db_stats.get('safe_images', 0)}")
...
def format_analysis_output(analysis) -> str:
    ...
    if languages:
        ...
        for lang in languages:
            ...
            output.append(f"   {verified} {lang['language']} {lang.get('version', 'unknown')}")
    ...
    if vulnerability_details:
        ...
        for vuln in vulnerability_details[:5]:
            ...
            output.append(f"   {severity_emoji} {vuln_id} - {package}:{version} ({severity})")
    ...
    try:
        ...
        if scanner.db.image_exists(args.scan_image):
            if not getattr(args, 'update_existing', False):
                ...
                print(f"   Example: python src/cli.py --scan-image {args.scan_image} --update-existing --comprehensive")
                scanner.db.close()
                return 1
            else:
        ...
        force_update = getattr(args, 'update_existing', False)  # Respect the --update-existing flag
...
def get_docker_manifest_data(image_name: str) -> Optional[Dict]:
    ...
    try:
        ...
        for variant in image_name_variations:
            ...
            if images_result.returncode == 0 and images_result.stdout.strip():
                ...
                try:
                    ...
                    print(f"   📏 Parsed size: {actual_size / (1024*1024):.1f} MB from docker images (using name: {variant})")
        ...
        if actual_size == 0:
            print(f"   ⚠️  Failed to get size from docker images for all name variations, setting size to 0")
        ...
        if actual_size > 0:
            print(f"   📊 Size: {actual_size / (1024*1024):.1f} MB, Layers: {manifest_data['layers']}")
        else:
