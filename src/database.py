...
class ImageDatabase:
    ...
    def verify_table_constraints(self) -> bool:
        ...
        try:
            ...
            cursor = self.conn.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='images'")
    ...
    def image_exists(self, image_name: str) -> bool:
        ...
        cursor = self.conn.execute(
            "SELECT 1 FROM images WHERE name = ? LIMIT 1",
            (image_name,)
        )
    ...
    def check_image_needs_update(self, analysis_data: Dict) -> Tuple[bool, str]:
        ...
        if new_scan_timestamp and existing_scan_timestamp:
            try:
                from datetime import datetime
                new_dt = datetime.fromisoformat(new_scan_timestamp.replace('Z', '+00:00'))
                existing_dt = datetime.fromisoformat(existing_scan_timestamp.replace('Z', '+00:00'))
                
                if new_dt <= existing_dt:
                    return False, f"Existing scan is newer or same ({existing_scan_timestamp} >= {new_scan_timestamp})"
        ...
        if abs(new_vuln_total - existing_vuln_total) > 0:
            return True, f"Vulnerability count changed ({existing_vuln_total} -> {new_vuln_total})"
    ...
    def insert_image_analysis(self, analysis_data: Dict, force_update: bool = False) -> int:
        """
        ...
        if not force_update:
            ...
            if not needs_update:
                ...
                existing_info = self.get_image_scan_info(image_name)
                if existing_info:
                    cursor = self.conn.execute("SELECT id FROM images WHERE name = ?", (image_name,))
                    row = cursor.fetchone()
        ...
        if isinstance(layers_value, list):
            ...
            print(f"    🐛 DEBUG: layers is not a number: {layers_value} (type: {type(layers_value)}), setting to 0")
            layers_value = 0
        ...
        for key, value in analysis_data.items():
            if key == 'layers' and isinstance(value, list):
                print(f"    🐛 DEBUG: Found 'layers' as list in analysis_data['{key}']: {value}")
            if isinstance(value, dict):
                for subkey, subvalue in value.items():
                    if subkey == 'layers' and isinstance(subvalue, list):
                        print(f"    🐛 DEBUG: Found 'layers' as list in analysis_data['{key}']['{subkey}']: {subvalue}")
        ...
        params = [
            ...
            str(vulnerabilities.get('scan_timestamp', '')) if vulnerabilities.get('scan_timestamp') else None, # 18
            str(vulnerabilities.get('scanner', '')) if vulnerabilities.get('scanner') else None, # 19
            ...
            str(comprehensive_security.get('scan_timestamp', '')) if comprehensive_security.get('scan_timestamp') else None, # 23
            str(comprehensive_security.get('scanner', '')) if comprehensive_security.get('scanner') else None # 24
        ]
        ...
        clean_params = [
            ...
            params[18] if params[18] is not None else None,               # 18: vulnerability_scan_timestamp
            ...
            params[23] if params[23] is not None else None,               # 23: comprehensive_scan_timestamp
        ...
        try:
            ...
            if "UNIQUE constraint failed" in str(e):
                ...
                cursor = self.conn.execute("""
                    UPDATE images SET 
                        registry = ?, repository = ?, tag = ?, digest = ?, size_bytes = ?, layers = ?,
                        created_date = ?, scan_timestamp = ?, base_os_name = ?, base_os_version = ?,
                        total_vulnerabilities = ?, critical_vulnerabilities = ?, high_vulnerabilities = ?,
                        medium_vulnerabilities = ?, low_vulnerabilities = ?, negligible_vulnerabilities = ?,
                        unknown_vulnerabilities = ?, vulnerability_scan_timestamp = ?, vulnerability_scanner = ?,
                        ...
                        comprehensive_scan_timestamp = ?, comprehensive_scanner = ?
                    WHERE name = ?
                """, clean_params[1:] + [clean_params[0]])  # Move name to end for WHERE clause
                
                # Get the existing image ID
                cursor = self.conn.execute("SELECT id FROM images WHERE name = ?", (clean_params[0],))
                row = cursor.fetchone()
        ...
        for i, lang_data in enumerate(analysis_data.get('languages', [])):
            try:
                ...
                if cursor.rowcount > 0:
                    ...
                    print(f"    ⚠️  Warning: Language insert {i+1} did not insert any rows: {lang_data}")
            except Exception as e:
                print(f"    ❌ Error inserting language {i+1} ({lang_data.get('language', 'unknown')}): {e}")
        ...
        for i, pm_data in enumerate(analysis_data.get('package_managers', [])):
            try:
                ...
                if cursor.rowcount > 0:
                    ...
                    print(f"    ⚠️  Warning: Package manager insert {i+1} did not insert any rows: {pm_data}")
            except Exception as e:
                print(f"    ❌ Error inserting package manager {i+1} ({pm_data.get('name', 'unknown')}): {e}")
        ...
        for i, capability in enumerate(analysis_data.get('capabilities', [])):
            try:
                ...
                if cursor.rowcount > 0:
                    ...
                    print(f"    ⚠️  Warning: Capability insert {i+1} did not insert any rows: {capability}")
            except Exception as e:
        ...
        for i, pkg_data in enumerate(analysis_data.get('system_packages', [])[:]):  
            try:
                ...
                if cursor.rowcount > 0:
                    ...
                    print(f"    ⚠️  Warning: System package insert {i+1} did not insert any rows: {pkg_data}")
            except Exception as e:
                print(f"    ❌ Error inserting system package {i+1} ({pkg_data.get('name', 'unknown')}): {e}")
        ...
        for i, vuln_data in enumerate(analysis_data.get('vulnerability_details', [])):
            try:
                ...
                if isinstance(fixed_version, list):
                    ...
                    fixed_version = ', '.join(str(v) for v in fixed_version) if fixed_version else None
                ...
                if isinstance(description, list):
                    description = '; '.join(str(d) for d in description) if description else None
                elif description is not None:
                    description = str(description)
                
                package_name = vuln_data.get('package_name')
                if isinstance(package_name, list):
                    package_name = ', '.join(str(p) for p in package_name) if package_name else None
                elif package_name is not None:
                    package_name = str(package_name)
                
                package_version = vuln_data.get('package_version')
                if isinstance(package_version, list):
                    package_version = ', '.join(str(v) for v in package_version) if package_version else None
                ...
                if cursor.rowcount > 0:
                    ...
                    print(f"    ⚠️  Warning: Vulnerability insert {i+1} did not insert any rows: {vuln_data}")
            except Exception as e:
                print(f"    ❌ Error inserting vulnerability {i+1} ({vuln_data.get('id', 'unknown')}): {e}")
        ...
        for i, finding_data in enumerate(analysis_data.get('security_findings', [])):
            try:
                ...
                if cursor.rowcount > 0:
                    ...
                    print(f"    ⚠️  Warning: Security finding insert {i+1} did not insert any rows: {finding_data}")
            except Exception as e:
                print(f"    ❌ Error inserting security finding {i+1} ({finding_data.get('title', 'unknown')}): {e}")
        ...
        if comprehensive_security and comprehensive_security.get('scanner'):
            ...
            for i, secret in enumerate(comprehensive_security.get('secret_details', [])):
                try:
                    cursor = self.conn.execute("""
                        INSERT INTO security_findings (
                            image_id, finding_type, severity, rule_id, title, 
                    ...
                    """, (
                        image_id, 'secret', secret.get('severity', 'unknown'),
                        secret.get('rule_id'), secret.get('title'),
                        secret.get('file_path'), secret.get('category')
                    ))
                    if cursor.rowcount > 0:
                        comprehensive_findings_inserted += 1
                    else:
                        print(f"    ⚠️  Warning: Secret finding insert {i+1} did not insert any rows: {secret}")
                except Exception as e:
                    print(f"    ❌ Error inserting secret finding {i+1} ({secret.get('title', 'unknown')}): {e}")
            
            # Insert configuration findings
            for i, config in enumerate(comprehensive_security.get('config_details', [])):
                try:
                    cursor = self.conn.execute("""
                        INSERT INTO security_findings (
                            image_id, finding_type, severity, rule_id, title,
                    ...
                    """, (
                        image_id, 'config', config.get('severity', 'unknown'),
                        config.get('check_id'), config.get('title'),
                        config.get('description'), config.get('message')
                    ))
                    if cursor.rowcount > 0:
                        comprehensive_findings_inserted += 1
                    else:
                        print(f"    ⚠️  Warning: Config finding insert {i+1} did not insert any rows: {config}")
                except Exception as e:
                    print(f"    ❌ Error inserting config finding {i+1} ({config.get('title', 'unknown')}): {e}")
            
            # Insert license findings
            for i, license_info in enumerate(comprehensive_security.get('license_details', [])):
                try:
                    cursor = self.conn.execute("""
                        INSERT INTO security_findings (
                    ...
                    """, (
                        image_id, 'license', 'info',
                        license_info.get('name'), license_info.get('file_path')
                    ))
                    if cursor.rowcount > 0:
                        comprehensive_findings_inserted += 1
                    else:
                        print(f"    ⚠️  Warning: License finding insert {i+1} did not insert any rows: {license_info}")
                except Exception as e:
                    print(f"    ❌ Error inserting license finding {i+1} ({license_info.get('name', 'unknown')}): {e}")
        
        if comprehensive_findings_inserted > 0:
            print(f"    ✅ Inserted {comprehensive_findings_inserted} comprehensive security findings")
        
        self.conn.commit()
        
        # Log summary of all inserts
        total_inserts = (languages_inserted + package_managers_inserted + capabilities_inserted + 
                        system_packages_inserted + vulnerabilities_inserted + security_findings_inserted + 
    ...
    def _clear_image_relations(self, image_id: int):
        ...
        tables = ['languages', 'package_managers', 'capabilities', 'system_packages', 'vulnerabilities', 'security_findings']
    ...
    def query_images_by_language(self, language: str, version: str = None, 
                                max_vulnerabilities: int = None) -> List[Dict]:
        ...
        if version:
            ...
            if version_match:
                ...
                params.extend([f"{version}%", f"{version}%", f"{major_minor}%", major_minor])
            else:
        ...
        base_query += " GROUP BY i.id ORDER BY i.total_vulnerabilities ASC, i.size_bytes ASC"
        
        cursor = self.conn.execute(base_query, params)
        return [dict(row) for row in cursor.fetchall()]
    
    def query_secure_images(self, max_critical: int = 0, max_high: int = 0) -> List[Dict]:
    ...
    def get_all_images_with_details(self, page: int = 1, per_page: int = 20, 
                                   language_filter: str = '', security_filter: str = 'all') -> List[Dict]:
        ...
        if language_filter:
            where_clauses.append("EXISTS (SELECT 1 FROM languages l WHERE l.image_id = i.id AND l.language = ?)")
            params.append(language_filter)
        
        if security_filter == 'secure':
            where_clauses.append("i.total_vulnerabilities = 0")
        elif security_filter == 'safe':
            where_clauses.append("i.critical_vulnerabilities = 0 AND i.high_vulnerabilities = 0")
    ...
    def get_image_count(self, language_filter: str = '', security_filter: str = 'all') -> int:
    ...
    def export_all_data(self) -> Dict:
        ...
        for image_row in cursor.fetchall():
            ...
            # Get related data
            cursor2 = self.conn.execute("SELECT * FROM languages WHERE image_id = ?", (image_id,))
            image_data['languages'] = [dict(row) for row in cursor2.fetchall()]
            
            cursor2 = self.conn.execute("SELECT capability FROM capabilities WHERE image_id = ?", (image_id,))
            image_data['capabilities'] = [row[0] for row in cursor2.fetchall()]
            
            cursor2 = self.conn.execute("SELECT * FROM package_managers WHERE image_id = ?", (image_id,))
