...
def populate_single_image(image_name: str, db_path: str = "azure_linux_images.db", 
    ...
    try:
        ...
        print(f"   Vulnerabilities: {analysis.get('vulnerabilities', {}).get('total', 0)}")
...
def populate_from_image_list(image_list_file: str, db_path: str = "azure_linux_images.db",
...
def migrate_from_json(json_file: str, db_path: str = "azure_linux_images.db") -> int:
    ...
    try:
        ...
        for i, image_data in enumerate(images, 1):
            try:
                print(f"[{i}/{len(images)}] Migrating: {image_data.get('image', 'unknown')}")
...
Examples:
  ...
  %(prog)s --info
    ...
    action.add_argument('--sample-db', action='store_true', help='Create sample database')
    action.add_argument('--migrate', metavar='JSON_FILE', help='Migrate from JSON database')
    ...
    try:
        if args.single:
            success = populate_single_image(args.single, args.db_path, args.comprehensive)
            sys.exit(0 if success else 1)
        
        elif args.image_list:
            count = populate_from_image_list(args.image_list, args.db_path, args.comprehensive)
